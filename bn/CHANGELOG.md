...
- ভাঙা ইনস্টলেশন ফ্রেম ঠিক করা হয়েছে। ড্যাশবোর্ড আর টার্মিনালের শেষ কলামে লেখে না,
  ফলে pending-wrap অবস্থা কখনো চালু হয় না এবং লগ লাইন বর্ডারের ভিতর দিয়ে মুড়ে গিয়ে
  পরের সারিগুলোকে সরিয়ে দিতে পারে না।
- অবিরাম ফুল-স্ক্রিন রি-পেইন্ট বন্ধ হয়েছে। resize পরীক্ষা এখন clamp করা মাপের বদলে
  আগের raw মাপের সঙ্গে নতুন raw মাপ মেলায়, এবং সেকেন্ডে সর্বোচ্চ একবার চলে (SIGWINCH
  এলে সঙ্গে সঙ্গে)। স্ক্রিন কেবল খোলার সময়, প্রকৃত resize-এ এবং বন্ধ করার সময় পরিষ্কার
  হয়।
- টার্মিনালের মাপ `stty`/`tput` দিয়ে কার্নেল থেকে পড়া হয়, তিনটি descriptor fallback
  সহ, যাতে pipeline-এ চলা ফেজও উইন্ডোর মাপ জানতে পারে; মাপ পাওয়া না গেলে ফ্রেম
  স্থির থাকে।
- apt, dpkg, proot-distro ও curl-এর carriage-return প্রগ্রেস এখন একটি সারিতেই
  হালনাগাদ হয়, হাজার হাজার লাইন হয়ে লগ উইন্ডো ভাসিয়ে দেয় না।
- আউটপুটের ঝড় পড়ার যোগ্য হারে সংক্ষেপ করা হয় এবং স্পষ্টভাবে `... +N more lines`
  লেখা হয়; error, warning ও phase চিহ্ন কখনো বাদ যায় না, আর পুরো স্ট্রিম সবসময় লগ
  ফাইলে লেখা হয়।
- আংশিক read বাফার করা হয়, ফলে ধীরে তৈরি হওয়া লাইন ভেঙে না গিয়ে জোড়া লাগে।
- প্রতি-লাইন রেন্ডার পথে আর কোনো fork নেই: clip, pad, রঙ ও অ্যানিমেটেড স্বাক্ষর কেবল
  shell builtin দিয়েই হিসাব হয়।
- resize-এ ফ্রেম ঠিক একবার নতুন মাপে বসে, আর টার্মিনাল খুব ছোট হয়ে গেলে ভাঙা বাক্স না
  এঁকে পরিষ্কারভাবে সাধারণ স্ক্রলিং আউটপুটে নেমে আসে।
- ইনস্টলার বন্ধ করার সময় তিনটি সারি রাখা হয় ফলাফল ও লগ পাথসহ scrollback-বান্ধব
  সারসংক্ষেপের জন্য, এবং scroll region ও cursor সবসময় ফেরত দেওয়া হয় — SIGINT/SIGTERM
  সহ।
- নতুন রেন্ডারার নিয়ন্ত্রণ: `--ui auto|dashboard|plain|off`, `--plain` ও `TERNUX_UI`;
  `--no-anim` এখন কেবল অ্যানিমেশন থামায়, লেআউট বদলায় না। পরীক্ষার জন্য
  `TERNUX_COLS`/`TERNUX_ROWS` দিয়ে geometry ঠিক করা যায়।
- যোগ হয়েছে `tests/ui_render.py`: ইনস্টলারকে সত্যিকারের pseudo terminal-এ চালিয়ে
